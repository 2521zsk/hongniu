...
Page({
	...
	formSubmit: function (e) {
        ...
        if (result) {
            common.ajaxRequestWork("did15", param, function (rec) {
                wx.showToast({
                    title: '提交成功',
                    icon: 'success',
                    duration: 1500
                ...
                setTimeout(function () {
					wx.redirectTo({
                        url: '../dataupload/' + that.data.ActivityType + '5?Guid=' + that.data.Guid + "&ActivityID=" + that.data.ActivityID + "&ActivityType=" + that.data.ActivityType
                    })
                }, 1500)
            })
		} else {
