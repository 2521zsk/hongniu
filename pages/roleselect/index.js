...
Page({
    ...
    cList: function () {
        ...
        wx.request({
			url: encodeURI(common.xbApi.apiUrl + common.xbApi.commonurl + common.xbApi.api["did2_34"]),
			data: param,
			method: 'post',
			dataType: 'json',
			header: {
			...
			complete: function (res) {
				if (JSON.stringify(res.data.resp_data.pms_t_tmonpromotionexecute) != "[]") {
					var datalist = res.data.resp_data.pms_t_tmonpromotionexecute;
					that.setData({
						cData: datalist
					})
				} else {
    ...
    carryoutView: function (event) {
        ...
        wx.navigateTo({
			url: '../carryout/list?uid=' + uid + "&StartDate=" + that.data.StartDate + "&EndDate=" + that.data.EndDate
        })
    },
